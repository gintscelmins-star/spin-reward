-- 00010_fix_rpc_type_casts.sql
-- Discovered during test run: spins.qr_token is uuid type, spins.session_id is text type.
-- RPC functions had two type mismatches:
--   1. manual_redeem_spin: WHERE qr_token = p_qr_token (uuid = text)
--   2. get_redemptions + manual_redeem_spin: JOIN sessions ON se.id = sp.session_id (uuid = text)
-- Fix: change p_qr_token parameter to uuid; add ::uuid cast on sp.session_id in JOINs.

-- ── manual_redeem_spin ────────────────────────────────────────────────────────
-- Must DROP first because we are changing parameter types (text → uuid)
...
BEGIN
  ...
  IF v_status = 'redeemed' THEN
    RETURN QUERY
      SELECT 'already_redeemed'::text, bk.booking_ref, sp.redeemed_at
      FROM spins sp
      LEFT JOIN sessions  se ON se.id = sp.session_id::uuid
  ...
  UPDATE spins SET status = 'redeemed', redeemed_at = now() WHERE id = v_spin_id;

  RETURN QUERY
    SELECT 'redeemed'::text, bk.booking_ref, now()
    FROM spins sp
    LEFT JOIN sessions  se ON se.id = sp.session_id::uuid
    LEFT JOIN bookings  bk ON bk.id = se.booking_id
    WHERE sp.id = v_spin_id;
...
GRANT EXECUTE ON FUNCTION manual_redeem_spin(uuid, uuid) TO authenticated;

-- ── get_redemptions ───────────────────────────────────────────────────────────
-- Fix the session JOIN cast and qr_token explicit cast
...
BEGIN
  ...
  ORDER BY sp.spun_at DESC NULLS LAST
  LIMIT  p_limit
  OFFSET p_offset;
END;
$$;

GRANT EXECUTE ON FUNCTION get_redemptions(uuid, timestamptz, timestamptz, uuid, uuid, int, int) TO authenticated;
